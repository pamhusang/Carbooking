                    ...
                    if ($index) {
                        if ($save['number'] == '') {
                            ...
                            foreach ($request->getUploadedFiles() as $item => $file) {
                                ...
                                if ($file->hasUploadFile()) {
                                    if (!File::makeDirectory($dir)) {
                                        ...
                                        try {
                                            $file->resizeImage(self::$cfg->car_img_typies, $dir, $save['id'].self::$cfg->stored_img_type, self::$cfg->car_w);
                                        } catch (\Exception $exc) {
                                            // ไม่สามารถอัปโหลดได้
                            ...
                            if (empty($ret)) {
                                ...
                                $db->delete($meta_table, ['vehicle_id', $save['id']], 0);
                                ...
                                $ret['location'] = $request->getUri()->postBack('index.php', ['module' => 'car-setup']);
                                // เคลียร์
                                $request->removeToken();
                            }
                        }
