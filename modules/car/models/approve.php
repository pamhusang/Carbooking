...
class Model extends \Kotchasan\Model
{
    ...
    public static function get($id)
    {
        ...
        $query = static::createQuery()
            ...
            ->join('user U', 'LEFT', ['U.id', 'R.member_id'])
            ->where(['R.id', $id]);
        $select = [
            'R.*',
            'U.name',
        ...
        foreach (Language::get('CAR_OPTIONS', []) as $key => $label) {
            $query->join('car_reservation_data M'.$n, 'LEFT', [['M'.$n.'.reservation_id', 'R.id'], ['M'.$n.'.name', $key]]);
                ...
                if ($index) {
                    ...
                    if ($canApprove == -1) {
                        ...
                        if ($save['vehicle_id'] == 0) {
                            ...
                            $ret['ret_vehicle_id'] = Language::replace('Search :name and select from the list', [':name' => 'Vehicle']);
                        }
                        if ($save['travelers'] == 0) {
                        ...
                        if (empty($ret)) {
                            ...
                            $db->delete($table, ['reservation_id', $index->id], 0);
                            ...
                            // location
                            $ret['location'] = $request->getUri()->postBack('index.php', ['module' => 'car-report', 'status' => $index->status]);
                            // เคลียร์
                            $request->removeToken();
                        }
                    }
